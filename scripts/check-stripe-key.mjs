...
const publishableKey =
	process.env.STRIPE_PUBLISHABLE_KEY?.trim() ?? process.env.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY?.trim();
...
(async () => {
	...
	console.log(`   Publishable Key:${publishableKey ? ` ${sanitizeKey(publishableKey)}` : ' n/a'}`);
	...
	try {
		...
		const displayName =
			account.settings?.dashboard?.display_name ?? account.business_profile?.name ?? 'n/a';
