...
beforeAll(async () => {
  ...
  try {
    ...
    // Seed the database using prisma db seed (which now has seed command configured)
    try {
      ...
      console.warn('⚠️ Seed failed, tests may have limited data:', seedErr);
      // Continue anyway - some tests may not need seed data
    }
  } catch (err) {
